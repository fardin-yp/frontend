...
<h1 style={{height:"10%" ,pointerEvents:"all"}}> ویرایش پروفایل 
<img onClick={() => setEdit(false)} 
style={{width:"20px",opacity:"0.7",position:"absolute" ,left:"-10px",top:"0px",cursor:"pointer"}} src="/images/cancel.png" alt="cancel"/>
</h1>
<form style={{height:"90%"}}>
<div style={{width:"70%",marginTop:"20px"}}>
<input style={{direction:"rtl"}} placeholder="نام جدید" onChange={(e) => setUserName(e.target.value) } />
</div>
<div style={{width:"70%",height:"80px" ,marginTop:"20px"}}>
<input style={{direction:"rtl"}} placeholder=" رمز عبور جدید" onChange={(e) => setPassword(e.target.value) } />
</div>
{error && <p style={{color:"red" ,fontSize:"12px",marginBottom:"-15px",marginTop:"-20px"}}>{error}</p>}
<button style={{background:"#03a9f4"}} onClick={editAdmin} disabled={loading} >ثبت ویرایش</button>
</form>
</div>
}
...
<div className="admin-profile">
   ...
   <h1>{info && info.find[0].username} خوش آمدید !</h1>
    <p style={{opacity:"0.8"}}> ایمیل :{info.email} </p>
   </label>

   </div>
   <div style={{width:"30%",flexFlow:"column",alignItems:"center"}}>
       <button style={{background:"#4caf50"}} onClick={() => {
       setEdit(true)}}>ویرایش پروفایل</button>
       {info && info.find[0].roll === "master" && <button onClick={() => setChange("newAdmin")}>ادمین جدید</button>}
   </div>
</div>
{info && info.find[0].roll === "master" &&  <div className={"allAdmin"}>
{info && info.allAdmin && info.allAdmin.map(res => {
