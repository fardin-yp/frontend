...
const Comment = ( {comments ,id ,link ,admin ,type ,reCaptcha} ) => {
    ...
    async function sendmessage(e){
     ...
     try{
        await axios.put(`http://dreamweb.runflare.run/comment/${link}`, message,{withCredentials:true}).then(res => {
...
useEffect(() => {
  ...
  const deleteComment = async (messId) => {
     setComments(comment.filter(res => res._id !== messId))
      ...
      axios.post(`http://dreamweb.runflare.run/adminRoute/removeComment/${type}` ,posts ,{withCredentials:true}).then(res => {
  ...
  const scrollToTop = () => {
    ...
    return (
        ...
        {slice && slice.map(mes => {
            return<div style={mes.answer && mes.answer !== "false" ? {marginRight:"5%" ,background:"#6e787f"} : null} key={mes._id} className={"mess"}>
                   {mes.answer && mes.answer !== "false" && <img id="reply-arrow" src="/uploads/reply.png" alt="reply" />}
                     <div className={mes.answer && mes.answer !== "false" ? "c-top-answer" : "c-top" } style={mes.admin ? {background:"#0d2040",color:"white"}:null}>
                        <p style={{fontSize:"13px" ,opacity:"0.5",marginRight:"0%"}}>{mes.time}</p>
                         <div>
                        {mes.answer && mes.answer !== "false" &&  <p 
                        style={ mes.answer && mes.answer !== "false"  ?{
                            fontSize:"14px",backgroundColor:"#6e787f",width:"max-content"
                            ,marginBottom:"4px 0px",padding:"2px 8px",color:"white",borderRadius:"5px"
                            } : {
